/*
Command queriesgen writes the canonical sqlc input for the saga schema, one file
per dialect, from saga/internal/queries.
...
check the statements the saga store executes against the schema saga/migrations
renders, at build time, with no database running. What the store executes is the
querier sqlc-gen-unison generates from these same files — the same text with the
...
with saga/migrations. `make unison` calls it that way before running the emitter
over both.
