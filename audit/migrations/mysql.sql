...
CREATE TABLE IF NOT EXISTS {{PREFIX}}entries (
    ...
    UNIQUE KEY {{PREFIX}}entries_chain_idx (scope, seq),
    KEY {{PREFIX}}entries_scope_time_idx (scope, recorded_at),
    KEY {{PREFIX}}entries_actor_idx (actor_id, recorded_at),
    KEY {{PREFIX}}entries_resource_idx (resource_type, resource_id, recorded_at)
);
