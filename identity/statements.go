...
// bind resolves a rendered statement's positional arguments, reporting the
// argument that had no value.
//
// Bind has no defaults, and that is the behavior worth keeping: every nullable
// argument here takes nil legitimately, so a missing name and an explicit nil
// are indistinguishable once bound. A miss is this package's bug rather than a
// caller's — nothing on a request path decides which arguments a statement has
// — so it is wrapped with the operation and reported rather than filled in.
func bind(statement querygen.Bound, values map[string]any, operation string) ([]any, error) {
