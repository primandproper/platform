/*
Command queriesgen writes the canonical sqlc input for the waitlist schema, one
file per dialect, from waitlists/internal/queries.
...
check the statements the waitlist store executes against the schema
waitlists/migrations renders, at build time, with no database running. What the
...
with waitlists/migrations. `make unison` calls it that way before running the
emitter over both.
