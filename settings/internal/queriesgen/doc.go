/*
Command queriesgen writes the canonical sqlc input for the settings schema, one
file per dialect, from settings/internal/queries.
...
check the statements the settings store executes against the schema
settings/migrations renders, at build time, with no database running. What the
...
with settings/migrations. `make unison` calls it that way before running the
emitter over both.
