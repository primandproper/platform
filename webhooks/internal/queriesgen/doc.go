/*
Command queriesgen writes the canonical sqlc input for the webhooks schema, one
file per dialect, from webhooks/internal/queries.
...
check the statements the webhooks store executes against the schema
webhooks/migrations renders, at build time, with no database running. What the
...
with webhooks/migrations. `make unison` calls it that way before running the
emitter over both.
