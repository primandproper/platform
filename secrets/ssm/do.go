...
// source.
//
// The concrete key is the point. A caller who has chosen AWS SSM Parameter
// Store can depend on the thing they chose rather than on the interface every
// provider shares, and so can reason about what this one alone does: reach one
// parameter prefix over the network, once per lookup. The interface key is an
// alias rather than a second provider, so both callers share one source and one
// API client.
//
...
// client because a nil one means "build one from the default credential chain"
// — which is what a container that registered none wants, and is not something
// a failed do.Invoke can say. The options because this package's remaining
// knobs live in secretscfg, which imports this package and so cannot be
// imported back.
//
...
func RegisterSecretSource(i do.Injector, client GetParameterAPI, opts ...Option) {
	...
	// says so in ssm.go — and the service it aliases was just provided.
