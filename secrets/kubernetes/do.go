...
// source.
//
// The concrete key is the point. A caller who has chosen the Kubernetes Secrets
// API can depend on the thing they chose rather than on the interface every
// provider shares, and so can reason about what this one alone does: reach the
// API server for a namespaced Secret, once per lookup. The interface key is an
...
// client because a nil one means "build one from the kubeconfig path or the
// in-cluster config" — which is what a container that registered none wants,
// and is not something a failed do.Invoke can say. The options because this
...
func RegisterSecretSource(i do.Injector, client SecretGetter, opts ...Option) {
	...
	// says so in kubernetes.go — and the service it aliases was just provided.
