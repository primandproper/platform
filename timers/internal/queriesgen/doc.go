...
One dialect, because timers serves one. The loop is still a loop over Dialects,
so the roster is a list rather than a decision written into the control flow — a
second dialect would be a schema and a line, not a rewrite.
