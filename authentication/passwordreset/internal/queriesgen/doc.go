/*
Command queriesgen writes the canonical sqlc input for the password reset token
schema, one file per dialect, from
authentication/passwordreset/internal/queries.
...
authentication/passwordreset/migrations renders, at build time, with no database
running. What the store executes is the querier sqlc-gen-unison generates from
these same files — the same text with the consumer's table prefix substituted for
