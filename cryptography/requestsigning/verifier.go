...
// NewVerifier builds the v1 Verifier: it reads SignatureHeader and checks it
...
// VerifyRequest checks the signature header against body.
//
// The TimestampHeader is not consulted. It is a courtesy copy for a receiver
// that wants to shed a stale request before reading a body at all; the value
// this checks is the one inside the signed material, which is the only one an
// attacker cannot edit.
func (v *verifier) VerifyRequest(ctx context.Context, header http.Header, body []byte) error {
	if header == nil {
