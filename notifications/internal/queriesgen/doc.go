/*
Command queriesgen writes the canonical sqlc input for the notifications schema,
one file per dialect, from notifications/internal/queries.
...
check the statements the notifications store executes against the schema
notifications/migrations renders, at build time, with no database running. What
the store executes is the querier sqlc-gen-unison generates from these same files
...
with notifications/migrations. `make unison` calls it that way before running the
emitter over both.
