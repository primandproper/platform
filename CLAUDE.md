...
**Implementation-specific injectors:** a provider package returns its own concrete type — `memory.NewInMemoryCache` returns `*memory.Cache[T]`, `postgres.NewDatabaseClient` returns `*postgres.Client`, `ssm.NewSecretSource` returns `*ssm.SecretSource` — never the interface it satisfies. Returning the interface is a lossy narrowing at the one point where the caller knows most: they picked this provider, and the interface hands back the union of every provider's failure modes and capabilities in exchange. A caller built on `*memory.Cache[T]` writes no `cache.ErrUnavailable` branch, because that cache has no network to lose; a caller built on `*postgres.Client` reads the native pgx pools off the value, without asserting for `PgxAccess`. A `var _ Iface = (*Impl)(nil)` next to the type keeps conformance a compile-time fact.

Each such package also carries its own `do.go`, registering under **both** keys: `do.Provide` on the concrete type, then `do.MustAs[*Impl, Iface](i)` to alias the interface onto it. Two keys, one instance — a second `do.Provide` returning the interface would build a second cache, a second connection pool, a second locker that contends with nothing. `do.MustAs` cannot fail here: the cast is what the `var _` assertion already proved, and the service being aliased was provided on the line above.

What these injectors invoke is what the package can genuinely resolve: its own `*Config`, a `context.Context`, a `database.Client`. Everything else is a parameter, because a container's copy of it is usually meant for something else — a `circuitbreaking.CircuitBreaker` in the container is far more often the one guarding the service's own inbound API than the one that should guard this cache (the reasoning `httpclient.RegisterHTTPClient` sets out), and a nil API client means "build the default from ambient credentials", which is not something a failed `do.Invoke` can say. Knobs that live in the `config` subpackage stay parameters too, since that subpackage imports the leaf and cannot be imported back; register through the config subpackage instead when the provider should be the config's to choose. The leaf `Register` is for the caller who has already chosen.
