...
Transport mappings split by tier. `errors/http` and `errors/grpc` map the primitives — `database`, `circuitbreaking`, `ratelimiting`, `idempotency`, `requestsigning`, the two search indexes, the `platformerrors` sentinels — and import those packages, so nothing in them may import `errors/http` or `errors/grpc` back. Everything built on top maps itself: `dataprivacy`, `links`, `operations` and `sessions` each export an `HTTPMapper` and a `GRPCMapper` beside their sentinels and import the two transport packages, and `service.Register` calls `RegisterHTTPErrorMapper`/`RegisterGRPCErrorMapper` for each. No `init()` — a mapper that installs itself by being linked in is a side effect a consumer cannot opt out of. A sentinel whose own wording is meant for the person reading it goes to `grpcerrors.RegisterClientSafeSentinels` as well, or gRPC sends the code's name instead.
