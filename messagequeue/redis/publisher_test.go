...
func buildRedisBackedPublisher(t *testing.T, cfg *Config, topic string) messagequeue.Publisher {
	...
	ctx := t.Context()
	provider, err := NewRedisPublisherProvider(*cfg)
	must.NoError(t, err)
...
func Test_redisPublisher_Stop(T *testing.T) {
	...
	T.Run("does not close the shared client used by other topics", func(t *testing.T) {
		...
		provider, err := NewRedisPublisherProvider(Config{QueueAddresses: []string{t.Name()}})
		must.NoError(t, err)
...
func TestNewRedisPublisherProvider(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		actual, err := NewRedisPublisherProvider(cfg)
		test.NoError(t, err)
		test.NotNil(t, actual)
	})

	T.Run("no addresses is an error, not a provider with a nil client", func(t *testing.T) {
		t.Parallel()

		// This used to build cleanly and panic on the first Publish.
		actual, err := NewRedisPublisherProvider(Config{})
		test.Error(t, err)
		test.Nil(t, actual)
	})
}
